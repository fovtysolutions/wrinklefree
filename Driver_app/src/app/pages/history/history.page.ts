...
export class HistoryPage implements OnInit {
  ...
  getOrders(event: any, haveRefresh: any) {
    this.api.post_private('v1/orders/getDriverOrders', { "id": localStorage.getItem('uid'), "limit": this.limit * 10 }).then((data: any) => {
      console.log(data);
      this.dummy = [];
      this.newOrders = [];
      this.oldOrders = [];
      if (data && data.status && data.status == 200 && data.data) {
        data.data.forEach((element: any) => {
          ...
          if (((x) => { try { JSON.parse(x); return true; } catch (e) { return false } })(element.address)) {
            ...
            element.address = address.address + ' ' + address.house + ' ' + address.landmark + ' ' + address.pincode;
          }
